...
| **Unparseable bucket configuration** | `Records skipped, by reason` climbs on `no_config`, or `Drops per second, by reason` on the parse reason; the records are committed, not stuck | the worker reads a bucket's configuration again on a bounded retry before giving up on a record, because the configuration store can be catching up behind the populator. A record whose payload cannot be parsed at all is dropped and counted, never retried forever: one bad configuration cannot stall a partition, and it cannot stall the destinations that share it. Fix the bucket's notification configuration; the events published while it was broken are gone, and the counter says how many. A steady `no_config` rate with no broken bucket means the populator is publishing for buckets the configuration store does not have, which is a store problem. |
