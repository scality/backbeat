...
const { applyCompositeEnvOverrides, applyEnvOverrides } = require('./config/envOverrides');
...
class Config extends EventEmitter {
     ...
     */
    _parseConfig(config) {
        applyCompositeEnvOverrides(config);
        applyEnvOverrides(config, backbeatConfigJoi);

        const parsedConfig = joi.attempt(config, backbeatConfigJoi);

        // the configuration an extension is validated against: the extension
        // configurations are left out, as they are validated one after the
        // other, and an extension must not depend on another one
        const globalConfig = { ...parsedConfig };
        delete globalConfig.extensions;
        ...
        if (parsedConfig.extensions) {
            Object.keys(parsedConfig.extensions).forEach(extName => {
                ...
                if (extension.configValidator) {
                    const extConfig = parsedConfig.extensions[extName];
                    const validatedConfig =
                              extension.configValidator(globalConfig, extConfig);
