...
const { applyEnvOverrides, applyProbeServerPortOverride } = require('./config/envOverrides');
...
class Config extends EventEmitter {
     ...
     */
    _parseConfig(config) {
        applyEnvOverrides(backbeatConfigJoi, config);
        if (process.env.LIVENESS_PROBE_PORT) {
            applyProbeServerPortOverride(backbeatConfigJoi, config, process.env.LIVENESS_PROBE_PORT);
        }

        if (process.env.MONGODB_HOSTS) {
            config.queuePopulator = config.queuePopulator || {};
            config.queuePopulator.logSource = 'mongo';
            config.queuePopulator.mongo = config.queuePopulator.mongo || {};
            config.queuePopulator.mongo.replicaSetHosts = process.env.MONGODB_HOSTS;
        }

        if (process.env.REDIS_SENTINELS) {
            config.redis = config.redis || {};
            config.redis.name = process.env.REDIS_HA_NAME || 'mymaster';
            config.redis.sentinels = process.env.REDIS_SENTINELS;
        ...
        if (process.env.REDIS_LOCALCACHE_HOST) {
            config.localCache = config.localCache || {};
            config.localCache.host = process.env.REDIS_LOCALCACHE_HOST;
            config.localCache.port = 6379;
        }

        if (process.env.REDIS_LOCALCACHE_PORT) {
            config.localCache = config.localCache || {};
            config.localCache.port = process.env.REDIS_LOCALCACHE_PORT;
        }

        if (process.env.REDIS_PORT && !process.env.REDIS_SENTINELS) {
            config.redis = config.redis || {};
        ...
        if (process.env.MONGODB_AUTH_USERNAME &&
            ...
            config.queuePopulator = config.queuePopulator || {};
            config.queuePopulator.mongo = config.queuePopulator.mongo || {};
            config.queuePopulator.mongo.authCredentials = {
