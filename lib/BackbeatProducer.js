    ...
    */
    constructor(config) {
        ...
        if (config.maxMessages) {
            params['queue.buffering.max.messages'] = config.maxMessages;
        }
        if (config.statsIntervalMs) {
            params['statistics.interval.ms'] = config.statsIntervalMs;
        }

        // create a new producer instance
        this._producer = new Producer(params, {
        ...
        // this._producer.on('event.log', event => {
        //     console.log('----')
        //     console.log(event);
        // })
        this._producer.on('event.stats', stats => {
            let msg;
            try {
                msg = JSON.parse(stats.message);
            } catch (e) {
                console.log(`ERR in event.stats: ${e}`);
                return;
            }
            const info = {
                name: msg.name,
                leftover_msg_cnt: msg.msg_max - msg.msg_cnt,
                leftover_msg_size: msg.msg_size_max - msg.msg_size,
            }

            if (info.leftover_msg_cnt <= 10000 || info.leftover_msg_size <= 500000) {
                console.log('---> STATS: reaching low count or size')
    ...
    _onDeliveryReport(error, report) {
        ...
        // console.log(topic, partition, offset, key)
        if (sendCtx.pendingReportsCount === 0) {
            // console.log('sendCtx pendingReportsCount is 0')
    ...
    */
    ...
    _sendToTopic(topic, entries, cb) {
        ...
        try {
            ...
            // console.log('-> an err occurred so I am trying an explicit poll call');
            // this._producer.poll();
