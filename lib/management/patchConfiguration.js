...
const { management } = require('arsenal');
const { loadLocationsOverlay } = management.locations;
...
function updateLocations(conf, instanceAuth, logger, cb) {
    try {
        const locations = loadLocationsOverlay(
            conf.locations, instanceAuth, logger);
        ...
        config.setLocationConstraints(locationsWithReplicationBackend);
...
function patchConfiguration(overlayVersion, conf, instanceAuth, metadata,
ingestionEnabled, logger, cb) {
    ...
    if (conf && conf.locations) {
        return async.waterfall([
            ...
            next => {
                ...
                return updateLocations(conf, instanceAuth, logger, next);
            }
        ], cb);
    }
    return process.nextTick(cb, null);
