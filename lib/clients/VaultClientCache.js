...
class VaultClientCache {
     ...
     *   and in the profile (set through setHost()/setPort())
     */
     getClientWithAWSCreds(profileName, credsProvider) {
         return credsProvider()
            .then(creds => {
                ...
                const delay = creds.expiration ? creds.expiration - new Date() : 0;
                if (delay <= 0) {
                    // Already expired, return an error and let the caller retry with fresh creds.
                    // This should never happen in practice, as we just called the credential provider.
