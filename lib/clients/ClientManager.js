...
class ClientManager {
    ...
    constructor(params, logger) {
        ...
        if (this._transport === 'https') {
            this.s3Agent = new HttpsAgent.Agent({ keepAlive: false });
            this.stsAgent = new HttpsAgent.Agent({ keepAlive: false });
        } else {
            this.s3Agent = new HttpAgent.Agent({ keepAlive: false });
            this.stsAgent = new HttpAgent.Agent({ keepAlive: false });
        }
