...
function createClient(connectionString, options) {
    ...
    zkClient.on('disconnected', something => {
        console.log("disconnected emitted!!", something);
    });
    zkClient.on('expired', something => {
        console.log("expired emitted!!", something);
    });
    zkClient.on('authenticationFailed', something => {
        console.log("authenticationFailed emitted!!", something);
    });
