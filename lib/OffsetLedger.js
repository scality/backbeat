 ...
 *
 * Emits:
 * - 'empty' (topic): fired from onOffsetProcessed when the count of
 *   in-flight offsets for that topic transitions to zero. Used by
 *   BackbeatConsumer's rebalance handler to wake up its drain wait
 *   without polling.
 */
class OffsetLedger extends EventEmitter {
    constructor() {
        super();
        this._ledger = {};
    }
    ...
    onOffsetProcessed(topic, partition, offset) {
        ...
        const committableOffset =
            this._getPartitionCommittableOffset(partitionOffsets);
        if (this.getProcessingCount(topic) === 0 &&
            this.listenerCount('empty') > 0) {
            // Defer the emit: callers of onOffsetProcessed (e.g.
            // BackbeatConsumer.onEntryCommittable) still need to call
            // _consumer.offsetsStore() with committableOffset after we
            // return. If a 'empty' listener triggers the rebalance
            // unassign synchronously, offsetsStore would hit ERR__STATE
            // on the now-unassigned partition. Deferring to the next
            // tick lets the caller finish its synchronous work first.
            //
            // Re-check the count inside the deferred callback: between
            // scheduling and firing, the caller may have pushed new
            // entries into the ledger (e.g. via onOffsetConsumed),
            // making the emit obsolete.
            //
            // The listenerCount guard avoids the nextTick allocation in
            // the steady-state hot path when no rebalance is active.
            process.nextTick(() => {
                if (this.getProcessingCount(topic) === 0) {
                    this.emit('empty', topic);
                }
            });
        }
