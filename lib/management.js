...
function saveServiceCredentials(conf, params, instanceAuth) {
    // TODO use a proper account id in arn, for now we start with an
    // arbitrary id and increment for each service account we
    // register.
    // TODO query extensions for their service accounts needs

    let nextAccountId = 234456789012;
    serviceCredentials = {
        accounts: (conf.users || [])
            .filter(u => u.accountType === params.serviceAccount)
            .sort((u, v) => (u.accountType < v.accountType ? -1 : 1))
            .map(u => ({
                name: u.accountType,
                arn: `aws::iam:${nextAccountId++}:root`,
