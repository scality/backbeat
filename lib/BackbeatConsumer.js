...
class BackbeatConsumer extends EventEmitter {
    ...
    constructor(config) {
        ...
        // becoming idle, OffsetLedger becoming empty for our topic).
        // Set via this._setDrain(); the persistent ledger listener
        // installed below delegates through this flag, so we don't
        // need to add/remove ledger listeners on every rebalance.
        this._drainCallback = null;
        // Bumped on every rebalance event; a deferred un-assign
        // captured under an older value must not run.
        this._rebalanceId = 0;
        this._offsetLedger.on('empty', topic => {
            if (topic === this._topic && this._drainCallback) {
                this._drainCallback();
            }
        });
    ...
    */
    ...
    /**
     * Set (or clear) the rebalance drain callback. Fed by two signals:
     * the processing queue becoming idle (via TaskScheduler.setDrain)
     * and the OffsetLedger emitting 'empty' for this consumer's topic
     * (via the persistent listener installed in the constructor).
     *
     * Pass null to clear both signals; this is what the rebalance
     * handler does in its unassign closure and on the outer-timeout
     * disconnect path.
     *
     * @param {?function} cb - callback to invoke when either signal
     *   fires, or null to clear
     * @returns {undefined}
     */
    _setDrain(cb) {
        this._drainCallback = cb;
        this._processingQueue.setDrain(cb || (() => {}));
    }
    ...
    _onRebalance(err, assignment) {
        // A new rebalance supersedes whatever a previous revoke left
        ...
        if (err.code === kafka.CODES.ERRORS.ERR__ASSIGN_PARTITIONS) {
            ...
            const unassign = jsutil.once(status => {
                ...
                // Reset the drain signals (queue + ledger) and timer.
                this._setDrain(null);
                clearTimeout(this._drainProcessQueueTimeout);
                this._drainProcessQueueTimeout = null;
                ...
                const doUnassign = () => {
                    if (rebalanceId !== this._rebalanceId) {
                        // a newer rebalance landed while offsets were
                        // being published: the assignment is no longer
                        // ours to release
            ...
            // Single callback fed by both signals (queue idle + ledger
            // empty), wired via this._setDrain(). Either signal may
            // fire first depending on the path that had work pending;
            ...
            this._drainProcessQueueTimeout = setTimeout(() => {
                // Defensive: clear the drain callback before disconnect
                // so a late ledger 'empty' emit can't fire into a
                // half-torn-down consumer.
                this._setDrain(null);
