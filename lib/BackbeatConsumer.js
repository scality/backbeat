...
class BackbeatConsumer extends EventEmitter {
    ...
    constructor(config) {
        ...
        const configJoi = {
            ...
            replayDelayInSec: joi.number().allow(null),
        };
        ...
        const { zookeeper, kafka, topic, groupId, queueProcessor,
                ...
                bootstrap, logConsumerMetricsIntervalS, replayDelayInSec } = validConfig;
        ...
        this._isReplayEnabled = !!replayDelayInSec;
        // TODO: increase concurrency for replay consumers.
        // this._concurrency = concurrency;
        this._concurrency = this._isReplayEnabled ? 1 : concurrency;
        ...
        return this._consumer.consume(nNewConsumeRequests, (err, entries) => {
            ...
            if (!err) {
                entries.forEach(entry => {
                    ...
                    const howManyMSBeforeResume = (entry.timestamp + (this._replayDelayInSec * 1000) - Date.now());

                    // if (this._isReplayEnabled) {
                    //     setTimeout(() => {
                    ...
                    if (this._isReplayEnabled && howManyMSBeforeResume > 0) {
                        this._unsubscribe();
                        console.log('PAUSED!!!');
                        setTimeout(this.subscribe.bind(this), howManyMSBeforeResume);
                        return undefined;
                    }
