        ...
        if (err.code === kafka.CODES.ERRORS.ERR__ASSIGN_PARTITIONS) {
            ...
            this._drainProcessQueueTimeout = setTimeout(() => {
                ...
                if (process.env.CRASH_ON_REBALANCE_TIMEOUT === 'true') {
                    ...
                    // process exits; nothing reacts to the failed healthcheck
                    // the disconnect above trips. On K8s the liveness probe
                    // does, and this env var is never set there. Hard exit
                    // rather than a graceful stop: the stop path would wait
                    // on the very tasks that are stuck.
                    this._log.fatal('consumer stuck after rebalance drain ' +
                        'timeout, exiting so the process supervisor ' +
                        'restarts us',
                        { topic: this._topic, groupId: this._groupId });
