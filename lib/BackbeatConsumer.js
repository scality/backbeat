    ...
    */
    ...
    _onRebalance(err, assignment) {
        ...
        if (err.code === kafka.CODES.ERRORS.ERR__ASSIGN_PARTITIONS) {
            ...
            // librdkafka requires the assignment to be synchronised on an
            // arbitrary rebalance error, and the bump above just superseded
            // whatever revoke was pending. close() does this itself.
            if (!this._shuttingDown) {
    ...
    /**
     * Run a consumer call that must not abort the shutdown.
     *
     * @param {string} op - name, for logging
