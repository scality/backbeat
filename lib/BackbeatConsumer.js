...
class BackbeatConsumer extends EventEmitter {
    ...
    */
    ...
    _tryConsume() {
        ...
        this._log.info('consuming messages', {
            concurrency: this._concurrency,
            running: this._processingQueue.running(),
            length: this._processingQueue.length(),
            pending: this._nConsumePendingRequests,
            available: nNewConsumeRequests,
        });
    ...
    onEntryCommittable(entry) {
        ...
        this._log.info('marked committable entry', {
            entry: { topic, partition, offset,
                     key: key && key.toString(), timestamp },
            committableOffset,
            groupId: this._groupId,
