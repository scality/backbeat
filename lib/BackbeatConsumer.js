    ...
    _onRebalance(err, assignment) {
        if (err.code === kafka.CODES.ERRORS.ERR__ASSIGN_PARTITIONS) {
            ...
            const unassign = jsutil.once(status => {
                ...
                this._drainProcessQueueTimeout = null;
                clearInterval(this._ledgerPollTimer);
                this._ledgerPollTimer = null;
            ...
            // The OffsetLedger doesn't expose an "empty" event, so poll. The
            // poll interval is short relative to typical deferred-commit
            // latency (~1-2s, dominated by the status producer's poll cycle).
            this._ledgerPollTimer = setInterval(checkFullyDrained, 100);
