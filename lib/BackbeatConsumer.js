    ...
    _onRebalance(err, assignment) {
        if (err.code === kafka.CODES.ERRORS.ERR__ASSIGN_PARTITIONS) {
            ...
            this._drainProcessQueueTimeout = setTimeout(() => {
                ...
                // Let owners decide what to do when stuck (e.g. exit on
                // S3C, where nothing restarts a process on a failed
                // healthcheck). disconnect() above is guarded and
                // non-throwing, so this emit always runs.
