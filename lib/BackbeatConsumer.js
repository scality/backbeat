...
class BackbeatConsumer extends EventEmitter {
    ...
    constructor(config) {
        ...
        this._unassignInProgress = false;
    ...
    isReady() {
        ...
        if (this._bootstrap) {
            ...
            } else if (!this._consumer.assignments().length) {
                this._log.error('KafkaConsumer is not assigned any partition',
                    { topic: this._topic, groupId: this._groupId });
                cs = false;
            }
        }
    ...
    */
    ...
    _tryConsume() {
        if (this._unassignInProgress) {
            this._log.info('unsigned in progress, do not consume()');
            return undefined;
        }
    ...
    _onRebalance(err, assignment) {
        if (err.code === kafka.CODES.ERRORS.ERR__ASSIGN_PARTITIONS) {
            ...
            // Ensure we don't start processing any more entries unassign is complete
            this._unassignInProgress = true;
