...
class BackbeatConsumer extends EventEmitter {
    ...
    constructor(config) {
        ...
        this._ledgerEmptyListener = null;
    ...
    _onRebalance(err, assignment) {
        if (err.code === kafka.CODES.ERRORS.ERR__ASSIGN_PARTITIONS) {
            ...
            this._log.info('rdkafka.revoke', {
                ...
                ledgerProcessing: this._offsetLedger.getProcessingCount(this._topic),
            });
            ...
            const unassign = jsutil.once(status => {
                ...
                // Reset the timer and ledger drain listener
                clearTimeout(this._drainProcessQueueTimeout);
                this._drainProcessQueueTimeout = null;
                if (this._ledgerEmptyListener) {
                    this._offsetLedger.removeListener(
                        'empty', this._ledgerEmptyListener);
                    this._ledgerEmptyListener = null;
                }
            ...
            const checkFullyDrained = () => {
                if (isFullyDrained()) {
                    unassign(unassignStatus.DRAINED);
                }
            };
            // Two signals can complete the wait: the queue's drain
            // callback (in-flight task workers finish) or the ledger's
            // 'empty' event (deferred onEntryCommittable callbacks
            // land). Either may fire first depending on the path that
            // had work pending. Both call checkFullyDrained, which
            // only triggers unassign once both conditions hold.
            this._processingQueue.setDrain(checkFullyDrained);
            this._ledgerEmptyListener = ledgerTopic => {
                if (ledgerTopic === this._topic) {
                    checkFullyDrained();
                }
            };
            this._offsetLedger.on('empty', this._ledgerEmptyListener);
