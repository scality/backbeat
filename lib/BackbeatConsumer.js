...
class BackbeatConsumer extends EventEmitter {
    ...
    constructor(config) {
        ...
        const { zookeeper, kafka, topic, groupId, queueProcessor,
                ...
                bootstrap, logConsumerMetricsIntervalS, circuitBreaker } = validConfig;
    ...
    _initConsumer() {
        ...
        // S3C use 1m as zookeeper / kafka can start after backbeat and slowly
        this._consumer.connect({ timeout: 60000 }, (err) => {
            if (err) {
                this.emit('error', err);
            }
        });
