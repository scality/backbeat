...
class BackbeatConsumer extends EventEmitter {
    ...
    _initConsumer() {
        ...
        this._consumer.connect({ timeout: 10000 }, () => {
            const opts = {
                ...
                // node-rdkafka requests every topic unless told otherwise,
                // whatever the topic option says; see _tryConsume for why a
                // full metadata response is not harmless to a subscriber
                allTopics: false,
                timeout: 10000,
            };
    ...
    */
    ...
    _tryConsume() {
        ...
        return this._consumer.consume(nNewConsumeRequests, (err, entries) => {
            ...
            if (err || entries.length < nNewConsumeRequests) {
                ...
                //
                // The request must be for this topic only: node-rdkafka's
                // getMetadata() asks for every topic unless allTopics is
                // false, and librdkafka re-evaluates the subscription on each
                // full metadata response, briefly finding no matching topic
                // and rejoining the group. Issued once a second from this
                // retry loop, that rejoin cancels every join in flight, and
                // the consumer only gets its partitions once the session
                // timeout has passed: the 45 s start-up stall.
                if (err?.code === kafka.CODES.ERRORS.ERR_UNKNOWN_TOPIC_OR_PART &&
                    this._consumer.isConnected() && !this.isPaused()) {
                    this._consumer.getMetadata({
                        topic: this._topic,
                        allTopics: false,
                    }, () => {});
                }
    ...
    _pauseAssignments() {
        try {
            const assignments = this._safeAssignments();
    ...
    onEntryCommittable(entry) {
        ...
        try {
            ...
            // the client is between a failed rebalance and its recovery: the
            // offset stays in the ledger and is stored again with the next
            // processed entry, or the partition is re-consumed from the last
            // commit after the rebalance, so this is duplicates, never loss.
            // Throwing here would take the process down from a task callback.
            this._log.warn('could not store the committable offset', {
                method: 'BackbeatConsumer.onEntryCommittable',
                topic,
                partition,
                committableOffset,
                groupId: this._groupId,
    ...
    /**
     * The topics this consumer is subscribed to, or an empty list when the
     * client cannot say: not built yet, disconnected, or in the erroneous
     * state librdkafka enters around a failed rebalance, where
     * subscription() throws. Callers that used the raw call from the commit
     * path took the process down on that throw.
     *
     * @return {string[]} subscribed topics
     */
    _safeSubscription() {
        try {
            if (!this._consumer || !this._consumer.isConnected()) {
                return [];
            }
            return this._consumer.subscription() || [];
        } catch (err) {
            this._log.warn('could not read the consumer subscription', {
                method: 'BackbeatConsumer._safeSubscription',
                topic: this._topic,
                groupId: this._groupId,
                error: err.message,
            });
    ...
    /**
     * The partitions currently assigned to this consumer, or an empty list
     * when the client cannot say, for the same reasons as _safeSubscription
     *
     * @return {Object[]} assigned topic partitions
     */
    _safeAssignments() {
        try {
            if (!this._consumer || !this._consumer.isConnected()) {
            ...
            this._log.warn('could not read the consumer assignments', {
                method: 'BackbeatConsumer._safeAssignments',
                topic: this._topic,
                groupId: this._groupId,
                error: err.message,
            });
    ...
    resume(site) {
        ...
        if (this._safeSubscription().length === 0) {
            this._consumer.subscribe([this._topic]);
     ...
     */
    isPaused() {
        return this._safeSubscription().length === 0;
    }
        ...
        return async.waterfall([
            next => {
                if (this._consumer?.isConnected()) {
                    const subscription = this._safeSubscription();
                    // Read the assignment before the unsubscribe, and in the
                    // same tick, so that no rebalance callback can run in
                    // between: it says whether a revoke is still owed to us.
                    const assignments = this._safeAssignments();
                    if (subscription.length > 0) {
                        ...
                        if (assignments.length > 0) {
                            ...
                            // waiting for current jobs to complete as well as
                            // commit the latest offsets.
                            //
                            // Only when partitions are assigned: a subscription
                            // is set the moment subscribe() is called, while the
                            // assignment arrives with the group join, and
                            // librdkafka raises no rebalance callback at all for
                            // an unsubscribe from a consumer that holds no
                            // partition. Waiting for the revoke then never
                            // returns, and a consumer closed shortly after it
                            // subscribed never shuts down.
                            this.once('unassign', () => next());
                            return;
                        }
                        this._log.info('closing a consumer with no assigned ' +
                            'partition, nothing to revoke', {
                            method: 'BackbeatConsumer.close',
                            topic: this._topic,
                            groupId: this._groupId,
                        });
                    }
