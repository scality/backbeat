...
const safeJsonParse = require('../extensions/lifecycle/util/safeJsonParse');
...
class BackbeatConsumer extends EventEmitter {
    ...
    _init() {
        ...
        if (this._kafkaBacklogMetricsConfig) {
            console.log("init kafka backlog metrics")
            this._initKafkaBacklogMetrics();
        } else {
            console.log("kafka backlog metrics ready")
        ...
        return this._consumer.once('ready', () => {
            ...
            if (this._logConsumerMetricsIntervalS !== undefined) {
                this._consumer.on('event.stats', res => {
                    ...
                    const topicStats = statsObj.result.topics?.[this._topic];
    ...
    _checkIfReady() {
        if (this._consumerReady && this._kafkaBacklogMetricsReady) {
            if (this._bootstrap) {
                console.log("checkIfReady::bootstrap")
                if (!this._bootstrapping) {
                    console.log("checkIfReady::bootstraping false")
    ...
    */
    ...
    _bootstrapConsumer() {
        ...
        function consumeCb(err, messages) {
            console.log("consumeCb", err, messages)
            if (err) {
                return undefined;
            }
            messages.forEach(message => {
                const bootstrapId = JSON.parse(message.value).bootstrapId;
                console.log("bootstrapId", bootstrapId)
                if (bootstrapId) {
