...
class BackbeatConsumer extends EventEmitter {
    ...
    */
    ...
    /**
     * Default queue processor that processes entries
     * @param {Object} queueEntry - the queue entry to process
     * @param {Function} cb - callback
     * @return {undefined}
     */
    processQueueEntry(queueEntry, cb) {
        // Use backend validation by default
        return this.processEntryWithBackendValidation(queueEntry, cb);
    }

    /**
     * Legacy queue processor without backend validation
     * @param {Object} queueEntry - the queue entry to process
     * @param {Function} cb - callback
     * @return {undefined}
     */
    processQueueEntryLegacy(queueEntry, cb) {
        console.warn('DEPRECATED: processQueueEntryLegacy bypasses backend state validation. Use processQueueEntry instead.');
        return this._queueProcessor(queueEntry, cb);
    }

    /**
     * Backend-aware service status check that queries persistent storage
     * instead of relying on in-memory subscription state.
     * This provides the source of truth and avoids state inconsistencies.
     * @param {string} service - service name (e.g., 'crr', 'ingestion', 'lifecycle')
     * @param {string} location - location name
     * @param {Object} zkClient - ZooKeeper client (for ZK-based services)
     * @param {Object} mongoClient - MongoDB client (for Mongo-based services)
     * @param {Object} serviceConfig - service configuration
     * @param {Function} cb - callback(error, isEnabled)
     * @return {undefined}
     */
    getServiceStatusFromBackend(service, location, zkClient, mongoClient, serviceConfig, cb) {
        if (!serviceConfig[service] || serviceConfig[service].isMongo) {
            // MongoDB-based service - check MongoDB state
            this._getServiceStatusFromMongo(service, location, mongoClient, cb);
        } else {
            // ZooKeeper-based service - check ZooKeeper state
            this._getServiceStatusFromZk(service, location, zkClient, serviceConfig[service], cb);
        }
    }

    /**
     * Get service status from MongoDB
     ...
     * @return {undefined}
     */
    _getServiceStatusFromMongo(service, location, mongoClient, cb) {
        const collection = mongoClient.db().collection('locationStatus');
        
        collection.findOne(
            { _id: location },
            { projection: { [`value.${service}.paused`]: 1 } }
        ).then(doc => {
            const serviceData = doc && doc.value && doc.value[service];
            const isEnabled = !(serviceData && serviceData.paused === true);
            return cb(null, isEnabled);
        }).catch(err => {
            return cb(err);
        });
    }
    ...
    /**
     * Get service status from ZooKeeper
     * @param {string} service - service name
     * @param {string} location - location name
     ...
     * @return {undefined}
     */
    _getServiceStatusFromZk(service, location, zkClient, serviceConfig, cb) {
        const zkNamespace = serviceConfig.namespace;
        const zkStatePath = serviceConfig.statePath;
        const path = `${zkNamespace}${zkStatePath}/${location}`;

        zkClient.getData(path, (err, data) => {
            if (err) {
                if (err.name === 'NO_NODE') {
                    // Node doesn't exist - assume enabled by default
                    return cb(null, true);
                }
                return cb(err);
            }
            ...
            try {
                ...
                const isEnabled = !(state.paused === true);
                return cb(null, isEnabled);
            } catch (parseErr) {
                return cb(parseErr);
            }
    ...
    /**
     * Process queue entry with backend state validation
     * This ensures we always check the current backend state before processing
     * any entry, preventing race conditions where the consumer thinks it's enabled
     * but the backend state shows it's paused.
     * @param {Object} queueEntry - the queue entry to process
     * @param {Function} cb - callback
     * @return {undefined}
     */
    processEntryWithBackendValidation(queueEntry, cb) {
        // Extract service and location info from queue entry
        const service = this._extractServiceFromEntry(queueEntry);
        const location = this._extractLocationFromEntry(queueEntry);
        
        if (!service || !location) {
            // If we can't determine service or location, fall back to original logic
            return this._queueProcessor(queueEntry, cb);
        }

        // Check backend state before processing
        this.getServiceStatusFromBackend(
            service, 
            location, 
            this._zkClient, 
            this._mongoClient, 
            ...
            (err, isEnabled) => {
                if (err) {
                    // On error, log and fall back to original processing
                    this._logger.warn('Failed to check backend state, proceeding with processing', {
                        method: 'BackbeatConsumer.processEntryWithBackendValidation',
                        error: err.message,
                        service,
                        location,
                    });
                ...
                if (!isEnabled) {
                    // Service is paused according to backend - skip processing
                    this._logger.debug('Skipping entry processing - service is paused', {
                        method: 'BackbeatConsumer.processEntryWithBackendValidation',
                        service,
                        location,
                    });
                    return cb(); // Skip processing but don't fail
                }

                // Service is enabled - proceed with processing
                return this._queueProcessor(queueEntry, cb);
            }
        );
    }
    ...
    /**
     * Extract service name from queue entry
     * @param {Object} queueEntry - queue entry
     * @return {string|null} service name
     */
    _extractServiceFromEntry(queueEntry) {
        // Implementation depends on queue entry structure
        // This is a placeholder - actual implementation would need to be
        // customized based on the specific queue entry format
        if (this._topic.includes('crr')) return 'crr';
        if (this._topic.includes('lifecycle')) return 'lifecycle';
        if (this._topic.includes('ingestion')) return 'ingestion';
        return null;
    }

    /**
     * Extract location name from queue entry
     * @param {Object} queueEntry - queue entry
     * @return {string|null} location name
     */
    _extractLocationFromEntry(queueEntry) {
        // Implementation depends on queue entry structure
        // This is a placeholder - actual implementation would need to be
        // customized based on the specific queue entry format
        if (queueEntry && queueEntry.target && queueEntry.target.location) {
            return queueEntry.target.location;
        }
        if (queueEntry && queueEntry.locationConstraint) {
            return queueEntry.locationConstraint;
        }
        return null;
    }
