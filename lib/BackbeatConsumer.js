...
class BackbeatConsumer extends EventEmitter {
    ...
    isReady() {
        ...
        if (this._backlogMetrics) {
            if (this._zookeeper) {
                const zkState = this._zookeeper.getState();
                if (zkState.code !==
                        zookeeper.State.SYNC_CONNECTED.code) {
                    this._log.error('zookeeper reported bad state:' +
                        `${zkState.name} != SYNC_CONNECTED`);
                    zk = false;
                }
            }
        }
        ...
        if (!this._bootstrap) {
            ...
            if (!cs) {
                this._log.error('consumer reported bad state');
            }
        }
        return zk && cs;
    }
