    ...
    */
    subscribe(paused) {
        ...
        this._processingQueue = async.queue(
            (task, cb) => setTimeout(cb, 2000), this._concurrency);
    ...
    _tryConsume() {
        ...
        if (nNewConsumeRequests === 0 || this._isShuttingDown) {
            // processing pipeline is 100% busy or consumer is shutting down, do not attempt to consume
            return undefined;
        }
        this._processingQueue.push(1, () => {});
        ...
        return this._consumer.consume(nNewConsumeRequests, (err, entries) => {
            ...
            if (err || entries.length === 0) {
                this._log.debug(err ? `error, retry in 1s: ${err.message}` :
                    ...
                    { topic: this._topic, groupId: this._groupId });
                if (err && this._consumer.isConnected() && !this.isPaused()) {
    ...
    close(cb) {
        ...
        return async.series([
            next => {
                // stopping consumption of new messages while
                // waiting for current ones to be processed
                this._isShuttingDown = true;
                // waiting for all current jobs to end before exiting
                if (this._processingQueue && !this._processingQueue.idle()) {
                    this._log.info('draining queue...');
                    this._processingQueue.drain = () => {
                        this._log.info('drained processing queue');
                        return next();
                    };
                    return;
                }
