...
/**
 * Removes a config field, leaving alone a path holding no section to remove it
 * from. The field is deleted rather than set to `undefined`, which the schema
 * accepts alike but carries the key over into the validated configuration.
 *
 * @param {Object} config - configuration to update
 * @param {string[]} path - config path
 * @returns {undefined}
 */
function deleteField(config, path) {
    const parent = getField(config, path.slice(0, -1));
