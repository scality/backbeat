...
const serviceCredsJoi = joi.object({
    type: joi.alternatives().try('account').required(),
...
const authJoi = joi.object({
    ...
    vault: joi.object({
        ...
        serviceCredentials: serviceCredsJoi.required(),
        host: joi.string().required(),
        port: joi.number().greater(0).required(),
        ...
        adminCredentialsFile: joi.string().required(),
