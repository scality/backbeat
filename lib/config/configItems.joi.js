...
const logJoi = joi.object({
    logLevel: logLevelJoi.default('info').meta({ env: 'LEVEL' }),
    dumpLevel: logLevelJoi.default('error'),
}).default();

const logJoiOptional = joi.object({
    logLevel: logLevelJoi.default(joi.ref('$log.logLevel')).meta({ env: 'LEVEL' }),
    dumpLevel: logLevelJoi.default(joi.ref('$log.dumpLevel')),
}).optional();
...
const mongoJoi = joi.object({
    // MONGODB_HOSTS, through the alias of the section, is the historic name
    replicaSetHosts: joi.string().default('localhost:27017').meta({ env: 'HOSTS' }),
...
const kafkaJoi = joi.object({
    topic: joi.string().default('backbeat-oplog'),
    consumerGroupId: joi.string().default('backbeat-oplog-consumer-id'),
    logName: joi.string().default('s3-recordlog'),
});

const extensionKafkaJoi = joi.object({
    topic: joi.string(),
    consumerGroupId: joi.string(),
    logName: joi.string(),
});

const qpKafkaJoi = kafkaJoi.append({
    notification: extensionKafkaJoi,
    lifecycle: extensionKafkaJoi,
