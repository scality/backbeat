    ...
    _processReadRecords(params, batchState, done) {
        ...
        return async.waterfall([
            next => this.logConsumer.readRecords(readOptions, (err, res) => {
                ...
                console.log('res from reading records', res);
                return next();
            }),
            next => {
                console.log('check', config.queuePopulator.logSource.indexOf('ingestion'), this.logOffset);
                if (this.newIngestion && config.queuePopulator.logSource.indexOf('ingestion') > -1 && this.raftId === 0) {
                    this.remoteLogOffset = batchState.logRes.info.cseq;
                ...
                } else if (this.newIngestion && (config.queuePopulator.logSource.indexOf('ingestion') === -1 || this.raftId !== 0)) {
