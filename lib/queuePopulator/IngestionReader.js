...
class IngestionReader extends LogReader {
    constructor(params) {
        ...
        // TODO
        this._topic = ingestionConfig.topic;
    }
    ...
    _setupIngestionProducer(cb) {
        ...
        (err, decryptedKey) => {
            ...
            // TODO: setup producer first
            // Ideally, we should just create a single Producer and pass
            // instance to each IngestionReader
            return this._setupProducer(this._topic, cb);
            // return cb();
        });
    }
    ...
    _processReadRecords(params, batchState, done) {
        ...
        return async.waterfall([
            ...
            (initState, next) => {
                // initState.isStatusComplete = true;
    ...
    _processPublishEntries(batchState, done) {
        ...
        const myEntries = entriesToPublish[this._targetZenkoBucket];
        if (!myEntries || myEntries.length === 0) {
            return done();
        }
        return this._producers[this._topic].send(myEntries, err => {
            if (err) {
                logger.error(
            ...
            batchState.publishedEntries[this._topic] = myEntries;
