...
class IngestionReader extends LogReader {
    constructor(params) {
        const { zkClient, ingestionConfig, kafkaConfig, bucketdConfig, qpConfig,
            logger, extension, producer, metricsProducer, s3Config } = params;
        super({ zkClient, kafkaConfig, logConsumer: {}, logId: '',
                logger, extensions: [extension], metricsProducer });
        ...
        // single extension for ingestion. Avoid using this._extensions
    ...
    _readInitState(logger, done) {
        ...
        return done(null, { isStatusComplete: true });
    ...
    processLogEntries(params, done) {
        ...
        async.waterfall([
            ...
            next => this._processPrepareAndPublishEntries(batchState, next),
    ...
    /*
        TODO: flush mechanism
        if IQP.getEntries() still has entries in queue but we are at a point
        where we stop reading logs because we are "caught up" with offset and
        no new entries are being queued from source, we will never reach this
        point.
        Maybe add an internal cron for checking + sending? or maybe too heavy
        just for this use case
    */

    _publishEntries(batchState, entries, logger, cb) {
        let flag = false;
        async.retry({
            interval: retryCount => 50 * Math.pow(2, retryCount),
            errorFilter: err => {
                console.log('--> In Error Filter:', err)
                if (err.description === 'Local: Queue full') {
                    flag = true;
                    return true;
                }
                return false;
            ...
            this._producer.send(entries, err => {
                // console.log('&&&&&&&&>>>>>> RETRY publishEntries')
                if (err) {
                ...
                if (flag) {
                    console.log('&&&&&&&&&&>>>>>>>> RETRY occurred, flag triggered')
                }
                return done();
            });
        }, cb);
    ...
    _processPrepareAndPublishEntries(batchState, done) {
        const { logRes, logStats, logger, initState } = batchState;

        // if logRes.log is empty (empty listObjectVersions listing), skip
        if (!logRes.log) {
            return done();
        }
        // if initState, these entries came from a snapshot
        if (initState) {
            ...
            const entries = this._extension.getEntries();
            return this._publishEntries(batchState, entries, logger, err => {
                if (err) {
                    // problem producing to kafka
        ...
        logRes.log.on('data', record => {
            ...
            record.entries.forEach(entry => {
                ...
                if (record.db === this.bucket) {
                    ...
                    const entries = this._extension.getEntries();
                    this._publishEntries(batchState, entries, logger, err => {
                        if (err) {
                            console.log('=======')
                            console.log('ERR emitting:')
                            console.log(err)
                            logRes.log.emit('error', err);
                        } else {
                            totalCount += entries.length;
                        }
                        // TODO: can test emitting an empty error
                    });
                }
            });
        });
        logRes.log.on('error', err => {
            logger.error('error processing and filtering stream', {
                method: 'IngestionReader._processPrepareAndPublishEntries',
                error: err,
            });
            return done(errors.InternalError);
        });
        logRes.log.on('end', () => {
            ...
            this._publishMetrics(totalCount);
    ...
    _publishMetrics(count) {
        const location = this.getLocationConstraint();
        if (count > 0) {
            console.log(`-> ${count}`)
            const value = { [location]: { ops: count } };
