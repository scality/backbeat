...
class IngestionReader extends LogReader {
                    ...
                    (err, data) => {
                        if (err) {
                            logger.error('Error retrieving logs', { err,
                                ...
                                IngestionPopulatorMetrics.onIngestionSourceOp('getRaftLog', 'error');
                            return next(err);
                        }
                        ...
                        IngestionPopulatorMetrics.onIngestionSourceOp('getRaftLog', 'success');
                        return next();
                    });
                }
    ...
    _processLogEntry(batchState, record, entry) {
        ...
        if (!record.db) {
            this._extensions.forEach(ext => ext.filter({
                type: entry.type,
                bucket: entry.bucket,
                // removing the v1 metadata prefixes if they exist
    ...
    _processPrepareEntries(batchState, done) {
        ...
        logRes.log.on('end', () => {
            ...
            IngestionPopulatorMetrics.onIngestionLogSaved(
    ...
    _processPublishEntries(batchState, done) {
        ...
        IngestionPopulatorMetrics.onIngestionJournalProcessed(
            logStats.nbLogRecordsRead,
            this.getLocationConstraint(),
            this.raftId
        );
        ...
        return async.each(Object.keys(entriesToPublish), (topic, done) => {
            ...
            return this._producer.send(topicEntries, err => {
                if (err) {
                    ...
                    IngestionPopulatorMetrics.onIngestionKafkaPublish('error');
                    return done(err);
                }
                ...
                IngestionPopulatorMetrics.onIngestionKafkaPublish('success');
                IngestionPopulatorMetrics.onIngestionQueued(
                    topicEntries,
                    this.getLocationConstraint(),
                    this.raftId
                );
