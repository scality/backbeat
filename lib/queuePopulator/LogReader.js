...
class LogReader {
    ...
    _isSupportedOperation(record, entry) {
        // List of methods defined in Metadata/lib/protocol.json.
        if (record.method && record.method < 10) {
            if (!entry.type ||
                entry.type === 'put' ||
                entry.type === 'del' ||
                entry.type === 'delete') {
    ...
    _processLogEntry(batchState, record, entry, cb) {
        if (!this._isSupportedOperation(record, entry)) {
        ...
        async.eachSeries(this._extensions, (ext, next) => {
            const overheadFields = {
                ...
                versionId: entry.overhead && entry.overhead.versionId,
            };
            const entryToFilter = {
                type: entry.type,
                bucket: record.db,
