...
class LogReader {
    ...
    _readLogOffset(done) {
        // skip reading offset if offset is managed
        // externally (In Kafka for example)
        ...
        return this.zkClient.getData(pathToLogOffset, (err, data) => {
            ...
            if (data) {
                ...
                const logOffset = logOffsetNumber.toString();
                this.log.debug(
    ...
    _initializeLogOffset(done) {
        // TODO start at latest log offset on non-bucketd backends (dmd)
    ...
    processLogEntries(params, done) {
        ...
        async.waterfall([
            ...
            err => {
                ...
                // - many records have been skipped
     ...
     * for updating the offset in zookeeper.
     *
     * The base class implementation returns true. Subclasses should
     * override this method if the offset is managed externally.
     *
     * @returns {boolean} true if the offset is managed by the log reader
     */
    isOffsetManaged() {
        return true;
    }
