...
class LogReader {
    ...
    // This is a temp workaround to create notification specific zookeeper node
    // TODO: handle this issue in federation, by creating notif specific parent
    // nodes and assigning raft id's to notif specific zk nodes
    _getPathToLogOffset() {
        const notifExtension = this._extensions.find(ext => (
            ext instanceof NotificationQueuePopulator
        ));

        if (notifExtension) {
            return `/bucket-notification${this.pathToLogOffset}`;
        }

        return this.pathToLogOffset;
    }
