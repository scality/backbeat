...
class LogReader {
    ...
    setup(done) {
        ...
        this._readLogOffset((err, offset) => {
            ...
            console.log('THIS LOGOFFSET IS SET TO', this.logOffset);
    ...
    _readLogOffset(done) {
        ...
        this.zkClient.getData(pathToLogOffset, (err, data) => {
            ...
            if (data) {
                let logOffset;
                console.log('READING LOGOFFSET', pathToLogOffset, data.toString());
                if (this.config.queuePopulator.logSource === 'mongo') {
    ...
    _writeLogOffset(done) {
        ...
        this.zkClient.setData(
            ...
            err => {
                ...
                console.log('WE HAVE SAVED LOG OFFSET', this.pathToLogOffset, offsetString);
                this.log.debug(
                    'saved log offset',
                    { method: 'LogReader._writeLogOffset',
                        zkPath: pathToLogOffset,
    ...
    _processReadRecords(params, batchState, done) {
        ...
        this.logConsumer.readRecords(readOptions, (err, res) => {
            console.log('THIS.LOGOFFSET WHILE READING RECORDS', this.logOffset);
            if (err) {
                this.log.error(
    ...
    _processPublishEntries(batchState, done) {
        ...
        if (logRes.info.start === null) {
            console.log('LOGRES INFO IS NULL');
            console.log('logRes is', logRes);
            if (this.logOffset) {
        ...
        if (this.config.queuePopulator.logSource === 'mongo') {
            ...
            console.log('NEXT LOG OFFSET SET FOR MONGO', batchState.nextLogOffset);
        } else {
            batchState.nextLogOffset =
                logRes.info.start + logStats.nbLogRecordsRead;
            console.log('NEXT LOG OFFSET SET FOR OTHER', batchState.nextLogOffset);
        }
    ...
    _processSaveLogOffset(batchState, done) {
        console.log('SAVING LOG OFFSET IN LOGREADER', batchState.nextLogOffset);
