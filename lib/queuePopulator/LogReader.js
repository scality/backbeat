...
class LogReader {
    ...
    _filterEntries(batchState, record, cb) {
        ...
        if (notifExtension) {
            async.eachSeries(record.entries, (entry, next) => {
    ...
    _processFilterEntries(batchState, done) {
        ...
        if (!currentRecord || !currentRecord.entries) {
            return done();
        }
        ...
        async.series([
            ...
            next => this._filterEntries(batchState, currentRecord, next),
            next => {
