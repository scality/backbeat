...
class LogReader {
    ...
    setup(done) {
        ...
        this._readLogOffset((err, offset) => {
            ...
            console.log('offset in Setup', offset);
    ...
    _readLogOffset(done) {
        ...
        this.zkClient.getData(pathToLogOffset, (err, data) => {
            console.log('THIS PATH TO LOG OFFSET', this.pathToLogOffset);
            console.log('THIS IS THE DATA', data.toString());
            // console.log(this.zkClient.getData.toString());
            if (err) {
                if (err.name !== 'NO_NODE') {
                    this.log.error(
    ...
    processLogEntries(params, done) {
        console.log('sanity check: in LogReader.processLogEntries');
        const batchState = {
            logRes: null,
            logStats: {
                nbLogRecordsRead: 0,
    ...
    _processReadRecords(params, batchState, done) {
        ...
        // need to implement this method for mongo!!
        this.logConsumer.readRecords(readOptions, (err, res) => {
            ...
            res.log.on('data', data => {
                console.log('Backbeat LogReader DATA', data);
            });
            res.log.on('error', data => {
    ...
    _processPublishEntries(batchState, done) {
        ...
        console.log('logres.info in processPublish', logRes.info);
        console.log('logstats', logStats.nbLogRecordsRead);
