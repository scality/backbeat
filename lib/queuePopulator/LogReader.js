...
class LogReader {
    ...
    _setupProducer(topic, done) {
        console.log('setting up producer', topic);
        console.log('this._producers[topic]', this._producers[topic]);
        ...
        producer.once('ready', () => {
            console.log('ready signal emitted');
    ...
    _processPublishEntries(batchState, done) {
        ...
        return async.each(Object.keys(entriesToPublish), (topic, done) => {
            ...
            return async.series([
                ...
                done => this._producers[topic].send(topicEntries, (err, res) => {
                    console.log('sent? ', err, res);
                    return  done();
                }),
            ], err => {
                console.log('SET UP PRODUCERS', err);
                if (err) {
                    logger.error(
