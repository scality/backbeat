...
const localizationObjectMetrics = ZenkoMetrics.createCounter({
    name: 's3_backbeat_populator_localization_objects_total',
    help: 'Total objects queued for clean room localization',
    labelNames: metricLabels,
});

const localizationByteMetrics = ZenkoMetrics.createCounter({
    name: 's3_backbeat_populator_localization_bytes_total',
    help: 'Total number of bytes queued for clean room localization',
    labelNames: metricLabels,
});
 ...
 * @property {CounterInc} localizationObjects - Increments the localized objects metric
 * @property {CounterInc} localizationBytes - Increments the localized bytes metric
...
const metricsHandler = {
    ...
    localizationObjects: wrapCounterInc(localizationObjectMetrics, {}),
    localizationBytes: wrapCounterInc(localizationByteMetrics, {}),
    ...
    open(cb) {
        async.series([
            ...
            next => {
                ...
                startCircuitBreakerMetricsExport(this._circuitBreaker, 'queue_populator');
                return next();
            },
        ], err => {
            if (err) {
        ...
        switch (this.qpConfig.logSource) {
            ...
            default:
                throw new Error("bad 'logSource' config value: expect 'bucketd'," +
                    ` 'kafka' or 'dmd', got '${this.qpConfig.logSource}'`);
        }
    }
