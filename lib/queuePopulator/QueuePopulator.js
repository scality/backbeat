...
class QueuePopulator {
     ...
     *
     * @param {import ('http').HTTPServerResponse} res
     * @param {Logger} log
     */
    handleLiveness(res, log) {
        ...
        if (zkState.code !== zookeeper.State.SYNC_CONNECTED.code) {
            responses.push(`Zookeeper is not connected: ${state}`);
        }
        for(const reader of this.logReaders) {
            const prodStatuses = reader.getProducerStatus();
            for (const [topic, status] in Object.entries(prodStatuses)) {
                verboseLiveness[JSON.stringify(reader.getLogInfo())] = status;
                if (!status) {
                    responses.push(`log reader topic '${topic}' is not ready`);
                }
            }
        }
