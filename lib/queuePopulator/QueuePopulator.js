...
class QueuePopulator extends EventEmitter {
    /**
    ...
    _setupLogSources() {
        ...
        // using the config values of zookeeper path, zookeeper suffix,
            ...
            this.iConfig.sources.forEach(sourceObj => {
                ...
                console.log('ZK ENDPOINT', zkEndpointIngest);
    ...
    _subscribeToRaftSessionDispatcher(zkEndpoint, bucketd) {
        console.log('subscribing to raft session');
        this.raftIdDispatcher =
            new ProvisionDispatcher({ connectionString: zkEndpoint });
        console.log(zkEndpoint);
        console.log('still need to subscribe to raft session');
        console.log('console logging raftIdDispatcher', this.raftIdDispatcher);
        console.log('console logging all methods', typeof this.raftIdDispatcher);
        console.log('console logging all methods', Object.getOwnPropertyNames(this.raftIdDispatcher));
        console.log('console logging subscribe method', this.raftIdDispatcher.subscribe);
        this.raftIdDispatcher.subscribe((err, items) => {
            console.log('SUBSCRIBING TO RAFT ID DISPATCHER');
            this.emit('logReady');
            if (err) {
        ...
        return async.each(this._extensions, (ext, next) => {
            ...
            ext.setupZookeeper(err => {
                ...
                if (ext.createZkPath) {
                    ...
                    if (this.iConfig.sources) {
                        this.iConfig.sources.forEach(source => {
                            ext.createZkPath(next, source);
                        });
                    } else {
    ...
    _processAllLogEntries(params, done) {
        return async.map(
            ...
            (err, results) => {
                console.log('ERR RES for process all log entries');
