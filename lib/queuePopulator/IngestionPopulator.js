...
class IngestionPopulator {
    ...
    applyUpdates() {
        ...
        buckets.forEach(bucket => {
            ...
            if (isAlreadyActive) {
                ...
                // check/refresh the IngestionReader for any changes
                const hasRefreshed = this._ingestionSources[zenkoBucket]
                                         .refresh(sourceInfo);
                // if a change occurred, move back to `this.logReadersUpdate`
                // to re-validate and re-setup.
                // Otherwise, do nothing else with this bucket/location
                if (!hasRefreshed) {
                    return;
                }
                // Remove old reader with old configs, but keep zk state
                this._closeLogState(zenkoBucket, true);
            }
            // Get ingestion source information
            const sourceInfo = this._getSourceInformation(bucket);
    ...
    _setupUpdatedReaders(done) {
        ...
        this.logReadersUpdate = [];
        async.each(newReaders,
                   (logReader, cb) => logReader.setup(err => {
                       if (err) {
                           this.log.error('error has occurred on setup', {
                               error: err,
                               bucket: logReader.getTargetZenkoBucketName(),
                               location: logReader.getLocationConstraint(),
                           });
                           // log error and retry
                           this.logReadersUpdate.push(logReader);
                       } else {
                           // setup successfully, ready to read
     ...
     * @param {Boolean} [keepState] - if true, don't remove zookeeper state
     * @return {undefined}
     */
    _closeLogState(key, keepState) {
