...
class IngestionPopulator {
     ...
     */
    applyUpdates(cb) {
        return this._mongoClient.getIngestionBuckets((err, buckets) => {
            if (err) {
                return cb(err);
            }
            ...
            return async.series([
                    ...
                    if (!sources.includes(key)) {
                        ...
                        const auth = {
                            accessKey: locationDetails.accessKey,
                            secretKey: locationDetails.secretKey,
                        };
                        const newSource = {
    ...
    addNewLogSource(source, cb) {
        console.log(`addNewLogSource for ${source.name}`)
        return this._extension.createZkPath(err => {
            if (err) {
                return cb(err);
            }
    ...
    _subscribeToLogSourceDispatcher(zkEndpoint, bucketd) {
        console.log('_subscribeToLogSourceDispatcher')
        const key = bucketd.name;
        if (this._provisionedIngestionSources[key]) {
