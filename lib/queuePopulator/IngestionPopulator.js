...
const isOperatorBackend = backendName === 'operator';
...
class IngestionPopulator {
    ...
    _setupRedis(redisConfig) {
        ...
        redis.on('pmessage', (channel, pattern, message) => {
            console.log('REDIS pmessage => ', message);
            const validActions = {
    ...
    _getSourceInformation(ingestionInfo) {
        // ingestionInfo coming from pensieve push
        // {
        //      accessKey: ...,
        //      secretKey: ...,
        //      endpoint: 'http://10.100.1.128:8000',
        ...
        //      locationConstraint: 'my-ring',
        // }
        // ingestionInfo coming from operator
        // { 
        ...
        // TODO: have patchConfiguration() adapt locations the same way CloudServer does it.
        // We would then have one identical ingestionInfo.
        let {
            accessKey, secretKey, endpoint, locationType,
            bucketName, zenkoBucket, locationConstraint,
        } = ingestionInfo;

        if (isOperatorBackend) {
            const { credentials } = ingestionInfo
            accessKey = credentials.accessKey;
            secretKey = credentials.secretKey;

            const protocol = ingestionInfo.https ? 'https://' : 'http://';
            endpoint = `${protocol}${ingestionInfo.awsEndpoint}`;
        }
    ...
    _removePausedReaders() {
        console.log('this._pausedLocations!!!', this._pausedLocations);
