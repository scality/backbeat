...
class IngestionPopulator {
    ...
    _setupProducer(cb) {
        ...
        producer.once('ready', () => {
            ...
            producer.on('error', err => {
                ...
                console.log('-> IngestionPopulator producer.on error event')
            });
    ...
    close(done) {
        async.series([
            next => {
                if (this._mProducer) {
                    this.log.info('closing metrics producer', {
                        method: 'IngestionPopulator.close',
                    });
                    return this._mProducer.close(next);
                }
                this.log.info('no metrics producer to close', {
                    method: 'IngestionPopulator.close',
                });
                return next();
            },
            next => {
                if (this._mConsumer) {
                    this.log.info('closing metrics consumer', {
                        method: 'IngestionPopulator.close',
                    });
                    return this._mConsumer.close(next);
                }
                this.log.info('no metrics consumer to close', {
                    method: 'IngestionPopulator.close',
                });
                return next();
            },
            next => {
                if (this._producer) {
                    this.log.info('closing producer', {
                        method: 'IngestionPopulator.close',
                    });
                    return this._producer.close(next);
                }
                this.log.info('no producer to close', {
                    method: 'IngestionPopulator.close',
                });
                return next();
            },
    ...
    _processLogEntries(params, done) {
        return async.eachLimit(this.logReaders, 5,
            (logReader, cb) => {
                if (logReader.isBatchInProgress()) {
                    ...
                    // console.log(`\n----> batch in progress: ${logReader.getTargetZenkoBucketName()}`)
                    return cb();
                }
                return logReader.processLogEntries(params, error => {
                    if (error) {
                        ...
                        console.log('\n\n ERROR CAUGHT??')
                        console.log('====> ERROR IN populator._processLogEntries')
                    }
