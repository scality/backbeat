...
class IngestionPopulator {
     ...
     * @param {Object} [ingestionConfig.circuitBreaker] - circuit breaker
     * configuration
    ...
    open(cb) {
        ...
        async.series([
            ...
            next => {
                ...
                startCircuitBreakerMetricsExport(this._circuitBreaker, 'ingestion_populator');
                return next();
            },
        ], err => {
            if (err) {
    ...
    _pauseService(location) {
        // Check backend state instead of in-memory _pausedLocations
        this._getZkLocationState(location, (err, currentState) => {
            if (err) {
                this.log.error('error reading current state from zookeeper', {
            ...
            if (currentState.paused === true) {
                this.log.debug('location already paused according to backend state', {
                    method: 'IngestionPopulator._pauseService',
                    locationConstraint: location,
                });
                return;
            ...
            this._updateZkStateNodeTransactional(location, 'paused', true, err => {
                if (err) {
                    ...
                    // Sync in-memory state with backend
    ...
    _resumeService(location, date) {
        ...
        this._getZkLocationState(location, (err, currentState) => {
            ...
            if (currentState.paused !== true) {
                this.log.info(`cannot resume, location ${location} is not paused according to backend state`);
                return;
            }
            ...
            // Use transactional update with optimistic locking
            this._updateZkStateNodeTransactional(location, 'paused', false, err => {
                if (err) {
                    this.log.error('error occurred saving state to zookeeper', {
                        method: 'IngestionPopulator._resumeService',
                        locationConstraint: location,
    ...
    /**
     * Get current state for a location from ZooKeeper
     * @param {String} location - location constraint
     * @param {Function} cb - callback(error, state)
     * @return {undefined}
     */
    _getZkLocationState(location, cb) {
        const statePath = `${this.bucketInitPath}/../locations/${location}`;
        this.zkClient.getData(statePath, (err, data) => {
            if (err) {
                if (err.name === 'NO_NODE') {
                    // Node doesn't exist - assume not paused by default
     ...
     * @return {undefined}
     */
    _updateZkStateNodeTransactional(location, key, value, cb) {
        const statePath = `${this.bucketInitPath}/../locations/${location}`;
        
        // Use version-based optimistic locking
        this.zkClient.getData(statePath, (err, data, stat) => {
            if (err) {
                if (err.name === 'NO_NODE') {
                    // Create new state if doesn't exist
                    const newState = { [key]: value };
                    return this.zkClient.create(statePath, Buffer.from(JSON.stringify(newState)), cb);
                }
                this.log.error('could not get state from zookeeper', {
                    method: 'IngestionPopulator._updateZkStateNodeTransactional',
                    zookeeperPath: statePath,
                    error: err.message,
                });
                return cb(err);
            }
            ...
            try {
                ...
                if (state[key] === value) {
                    this.log.debug('state already has desired value, skipping update', {
                        method: 'IngestionPopulator._updateZkStateNodeTransactional',
                        key,
                        value,
                    });
                ...
                this.zkClient.setData(statePath, bufferedData, stat.version, err => {
                    if (err) {
                        if (err.name === 'BAD_VERSION') {
                            // Retry with exponential backoff
                            const retryDelay = Math.random() * 100; // 0-100ms
                            this.log.debug('version conflict, retrying update', {
                                method: 'IngestionPopulator._updateZkStateNodeTransactional',
                                retryDelay,
                            });
                            setTimeout(() => this._updateZkStateNodeTransactional(location, key, value, cb), retryDelay);
                            return;
                        }
                        this.log.error('could not save state data in zookeeper', {
                            method: 'IngestionPopulator._updateZkStateNodeTransactional',
                ...
                this.log.error('could not parse state data from zookeeper', {
                    method: 'IngestionPopulator._updateZkStateNodeTransactional',
                    zookeeperPath: statePath,
    ...
    /**
     * Check if location is paused according to backend state
     * MIGRATED: Now checks backend instead of in-memory _pausedLocations
     * @param {String} location - location constraint
     * @param {Function} cb - callback(error, isPaused)
     * @return {undefined}
     */
    isLocationPausedInBackend(location, cb) {
        this._getZkLocationState(location, (err, state) => {
            if (err) {
                return cb(err);
            }
    ...
    /**
     * DEPRECATED: Use isLocationPausedInBackend instead
     * Legacy method that checks in-memory _pausedLocations
     * @param {String} location - location constraint
     * @return {boolean} true if location is paused
     */
    isLocationPausedLegacy(location) {
        console.warn('DEPRECATED: isLocationPausedLegacy uses in-memory state. Use isLocationPausedInBackend for authoritative backend state.');
        return Object.keys(this._pausedLocations).includes(location);
    }
}
