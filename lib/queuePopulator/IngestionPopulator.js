...
class IngestionPopulator extends LogReader {
    ...
    _grabSnapshot(params, batchState, done) {
        return async.waterfall([
            next => {
                this.logConsumer.readRecords({}, (err, res) => {
                    if (err) {
                        this.log.error('error while reading log records', {
                            method: 'IngestionPopulator._grabSnapshot',
                            params, error: err });
                        return next(err);
                    }
                    this.log.debug('readRecords callback', {
                        method: 'IngestionPopulator._grabSnapshot',
