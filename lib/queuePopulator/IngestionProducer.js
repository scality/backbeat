...
const ringParams = {
    bucketdBootstrap: [config.ingestion.source1.host],
...
class IngestionProducer {
    ...
    _getBucketMd(bucketList, done) {
        return async.eachLimit(bucketList, 10, (bucket, cb) => {
            if (bucket.db === constants.usersBucket || bucket.db === constants.metastore) {
                return cb();
            }
            async.waterfall([
                next => {
                ...
                next => {
                    ...
                    return this.ringReader.getBucket(bucket.db,
    ...
    _getBucketObjects(bucketList, done) {
        return async.map(bucketList, (bucketInfo, cb) => {
            ...
            this.ringReader.listObject(bucketInfo.db, {}, this.requestLogger,
            (err, res) => {
                if (err) {
                    this.log.error('error listing object from bucket', {
                        method: 'IngestionProducer:getBucketObjects',
                        bucketName: bucketInfo.db,
                        err,
                    });
                    return done(err);
                }
                return cb(null, { bucket: bucketInfo.db, objects: res.Contents });
            });
