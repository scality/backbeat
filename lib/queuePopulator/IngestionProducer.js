...
class IngestionProducer {
    ...
    /**
     * Validate source bucket configuration and aws source credentials:
     * - credentials provided are accurate
     * - endpoint provided is accurate
     * - remote bucket exists
     * - remote bucket is version enabled
     * @param {string} bucket - remote bucket name
     * @param {Logger.newRequestLogger} logger - log object
     * @param {function} done - callback(error)
     * @return {undefined}
     */
    validateSourceSetup(bucket, logger, done) {
        this._s3Client.getBucketVersioning({ Bucket: bucket }, (err, res) => {
            if (err) {
                logger.error('error validating remote access, please check ' +
                'location configurations', {
                    method: 'IngestionProducer.validateRemoteAccess',
                    remoteBucket: bucket,
            ...
            if (!res.Status || res.Status !== 'Enabled') {
                logger.error('remote bucket is not version enabled', {
                    method: 'IngestionProducer.validateRemoteAccess',
                    remoteBucket: bucket,
                    zenkoBucket: this._targetZenkoBucket,
                    status: res.Status,
                ...
                return done(errors.InternalError.customizeDescription(
                    'remote bucket must be version enabled'));
            }
            return done();
        });
    }
    ...
    getRaftId(bucketName, done) {
        ...
        req.send((err, data) => {
            if (err) {
                let errMessage = `could not find bucket ${bucketName} in any` +
                    ' raft session';
                if (err.code === 'AccessDenied') {
                    errMessage = 'admin credentials are required';
                }
