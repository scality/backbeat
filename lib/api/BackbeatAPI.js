...
class BackbeatAPI {
    ...
    constructor(config, logger, optional) {
        ...
        this._redis = new RedisClient(this._localCacheRedis, this._logger);
        this._statsClient = new StatsModel(this._redis, INTERVAL,
            (EXPIRY + INTERVAL));
        const metricsConfig = {
    ...
    getCRRServiceStatus(details, cb) {
        ...
        async.each(sites, (site, next) => {
            ...
            if (error) {
                let errMessage = 'error getting crr service status';
