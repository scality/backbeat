...
class BackbeatAPI {
    ...
    constructor(config, logger, optional) {
        ...
        // only the methods below are of use here, but the instance owns a
        // redis client that has to be released on shutdown
    ...
    /**
     * Release the redis connections opened by the constructor
     * @return {undefined}
     */
    close() {
    ...
    _pushToCRRRetryKafkaTopics(queueEntry, cb) {
        ...
        const pendingEntry = queueEntry.toPendingEntry(backend);
        const retryEntry = queueEntry.toRetryEntry(backend);
        // Send messages in series to ensure status is updated in correct order.
        return async.series([
            next => this._crrStatusProducer
                .send([pendingEntry.toKafkaEntry(backend)], next),
            next => this._crrProducer
                .send([retryEntry.toKafkaEntry(backend)], next),
        ], err => {
            if (err) {
