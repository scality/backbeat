...
class BackbeatAPI {
     ...
     * Note: this has been changed to display pending metrics
     * @param {object} details - route details from lib/api/routes.js
     * @param {function} cb - callback(error, data)
     ...
     *   order of dataPoints mentioned for each route in lib/api/routes.js
     * @return {undefined}
     */
    ...
    getAllMetrics(details, cb, data) {
        ...
        this._getData(details, data, (err, res) => {
            ...
            return async.parallel([
                    ...
                    [res[1], res[4]]),
                done => this.getFailedMetrics(_adjustedDetails(details),
                    done, [res[2], res[5]]),
     ...
     * @param {string} site - site name or 'all'
    ...
    _queryStats(ops, site, cb) {
        return async.map(ops, (op, done) => {
            ...
            if (site === 'all') {
                const queryStrings = this._validSites.map(s => `${s}:${op}`);
                if (hasGlobalKey) {
