...
const PromAPIClient = require('../clients/PromAPIClient');
...
class BackbeatAPI {
    ...
    constructor(config, logger, optional) {
        ...
        const prometheusEndpoint =
              config.metrics && config.metrics.prometheusEndpoint;
        if (prometheusEndpoint) {
            this._promAPIHTTPAgent = new http.Agent({ keepAlive: true });
            this._promAPIClient = new PromAPIClient(
                prometheusEndpoint, this._promAPIHTTPAgent);
            this._lifecycleMetrics = new LifecycleMetrics(this._promAPIClient);
        }
    }
    ...
    findValidRoute(bbRequest) {
        ...
        if (route === 'metrics/lifecycle') {
            if (bbRequest.getHTTPMethod() !== 'GET') {
                return errors.MethodNotAllowed;
            }
            const route = routes.find(r => r.category === 'lifecycleMetrics');
            bbRequest.setMatchedRoute(route);
            return null;
        }
    ...
    getLifecycleMetrics(details, cb) {
        if (!this._lifecycleMetrics) {
            return process.nextTick(() => cb(
                errors.InternalError.customizeDescription(
                    'No configured Prometheus endpoint')));
        }
        const log = this._logger.newRequestLogger();
        return this._lifecycleMetrics.getMetrics(log, cb);
    }
