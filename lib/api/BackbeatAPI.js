...
class BackbeatAPI {
    ...
    /**
     * Delete all object-level metrics keys.
     * @param {String} bucket - The bucket that the key belongs to
     * @param {String} key - The key
     * @param {String} versionId - The key's version ID
     * @param {String} storageClass - The storage class of the CRR operation
     * @param {Function} cb - The callback to call
     * @return {undefined}
     */
    _deleteProgressKeys(bucket, key, versionId, storageClass, cb) {
        const keys = [
            getObjectBytesKey(storageClass, bucket, key, versionId),
            getObjectBytesDoneKey(storageClass, bucket, key, versionId),
        ];
        return async.each(keys, (key, next) =>
            this._redisClient.keys(`${key}*`, (err, res) => {
                if (err) {
                    return next(err);
                }
                return async.eachLimit(res, 10, (key, done) =>
                    this._redisClient.del(key, done),
                next);
            }),
        cb);
    }
                ...
                return async.series([
                    ...
                    // Delete the Redis lock key
                        ...
                        return async.parallel([
                            cb => this._redisClient.del(lockKey, cb),
                            cb => this._deleteProgressKeys(bucket, key,
                                versionId, storageClass, cb),
                        ], done);
                    },
                ], next);
            });
