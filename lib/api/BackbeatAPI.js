...
class BackbeatAPI {
    ...
    _processFailedKafkaEntries(entries, cb) {
        ...
        return async.eachLimit(entries, 10, (entry, next) => {
            ...
            return this._pushToCRRRetryKafkaTopics(queueEntry, err => {
                ...
                // If the original CRR was on a bucket without `versioning`
                // enabled (i.e. an NFS bucket), maintain the Redis key schema
                // by using and empty string.
    ...
    retryFailedCRR(details, body, cb) {
        ...
        const fields = reqBody.map(o => {
            const { Bucket, Key, StorageClass } = o;
            let { VersionId } = o;
            VersionId = VersionId === undefined ? '' : VersionId;
