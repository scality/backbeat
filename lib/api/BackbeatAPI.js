...
class BackbeatAPI {
    ...
    constructor(config, logger, optional) {
        ...
        this._expiry = config.metrics.expiryS;
        ...
        this._statsClient = new StatsModel(this._redisClient,
            config.metrics.intervalS, this._expiry);
    }
    ...
    getCompletions(details, cb, data) {
        this._getData(details, data, (err, res) => {
            ...
            // Find if time since start is less than this._expiry time
            const timeSinceStart = Math.floor(
                (Date.now() - this._internalStart) / 1000);
            // if timeSinceStart, then round the number to nearest 10's
            const timeDisplay = timeSinceStart < this._expiry ?
                (Math.ceil(timeSinceStart / 10) * 10) : this._expiry;
