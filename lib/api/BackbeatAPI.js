...
const { redisKeys, zookeeperReplicationNamespace, zkCRRStatePath } =
...
class BackbeatAPI {
    ...
    /**
     * After a request is made to change service state, this helper method is
     * used to check service state change. Once the desired state is identified,
     * return results to caller. There are max retry attempts.
     * @param {Array} sites - list of sites to check
     * @param {Object} condition - condition to meet
     * @param {Boolean} [condition.paused] - true/false
     * @param {Boolean} [condition.scheduledResume] - if false, expect
     *   all provided sites to delete scheduled resumes.
     *   if true, expect scheduled resume dates to change.
     * @param {Array} [condition.oldSchedules] - required for checking scheduled
     *   resumes. Old scheduled resumes will be compared to see if they change.
     * @param {Function} cb - callback(err, data) where data shows status for
     *   given sites
     * @return {undefined}
     */
    _checkServiceStateStatus(sites, condition, cb) {
        const storage = {};
        // dupe list of sites
        const s = sites.slice();
        // service state to check
        const type = Object.keys(condition).find(key =>
            key === 'paused' || key === 'scheduledResume');
        sites.forEach(site => { storage[site] = null; });

        return async.retry({
            times: 5, // retry for up to 5 times
            errorFilter: err => err.retry,
        }, done => this._getZkStateNodeDetails(s, (err, data) => {
            if (err) {
            ...
            // update storage & list of sites to check
            Object.keys(data).forEach(site => {
                storage[site] = data[site];

                // does paused state match condition?
                if (type === 'paused' && condition[type] === data[site][type]) {
                    s.splice(s.indexOf(site), 1);
                }
                // is scheduledResume cancelled?
                if (type === 'scheduledResume' && condition[type] === false &&
                data[site][type] === null) {
                    s.splice(s.indexOf(site), 1);
                }
                // did scheduledResume change for each site?
                if (type === 'scheduledResume' && condition[type] === true) {
                    const oldSchedule = condition.oldSchedules.find(s =>
                        Object.keys(s)[0] === site);
                    if (oldSchedule && oldSchedule[site] !== data[site][type]) {
                        s.splice(s.indexOf(site), 1);
                    }
                }
            });
            ...
            if (s.length > 0) {
                const err = Object.assign({},
                    errors.RequestTimeout.customizeDescription(
                        'zookeeper state did not update within time'),
                    { retry: true, sites: s });
                // still more to check
                return setTimeout(() => done(err), 1000);
            }
            return done();
        }), err => cb(err, storage));
    }
    ...
    pauseCRRService(details, body, cb) {
        ...
        return this._getZkStateNodeDetails(sites, (err, data) => {
            if (err) {
                return cb(err);
            }
            // if sites are already paused, log and return
            const alreadyPaused = sites.every(site => data[site].paused);
            if (alreadyPaused) {
                this._logger.info('replication service for location(s): ' +
                    `${sites} already paused`);
                const statuses = {};
                Object.keys(data).forEach(site => {
                    statuses[site] = 'disabled';
                });
                return cb(null, statuses);
            }

            // publish message to redis consumers
            sites.forEach(site => {
            ...
            // expect sites to be paused
            const condition = { paused: true };
            return this._checkServiceStateStatus(sites, condition,
            (err, data) => {
                if (err) {
                    const failedSites = err.sites;
                    this._logger.error('failed to pause replication service ' +
                        `for location(s): ${failedSites}`, {
                            error: err,
                            method: 'BackbeatAPI.pauseCRRService',
                        });
                    return cb(err);
                }

                this._logger.info('replication service paused for ' +
                    `location(s): ${sites}`);
                const statuses = {};
                Object.keys(data).forEach(site => {
    ...
    resumeCRRService(details, body, cb) {
        ...
        if (typeof details.schedule === 'boolean') {
            ...
            condition.scheduledResume = true;
        } else {
        ...
        return this._getZkStateNodeDetails(sites, (err, data) => {
            ...
            // if sites are already active, log and return
            const alreadyActive = sites.every(site =>
                data[site].paused === false);
            if (alreadyActive) {
                this._logger.info('replication service for location(s): ' +
                    `${sites} already active`);
                const statuses = {};
                Object.keys(data).forEach(site => {
                    statuses[site] = 'enabled';
                });
                return cb(null, statuses);
            }

            // set old scheduled times in `condition`
            condition.oldSchedules = sites.map(site => (
                { [site]: data[site].scheduledResume || null }
            ));
            ...
            (err, data) => {
                if (err) {
                    const failedSites = err.sites;
                    this._logger.error('failed to resume replication service ' +
                        `for location(s): ${failedSites}`, {
                            error: err,
                            method: 'BackbeatAPI.resumeCRRService',
                        });
                    return cb(err);
                }

                this._logger.info('replication service resumed for ' +
                    `location(s) ${sites}`);
                const statuses = {};
                Object.keys(data).forEach(site => {
                    if (condition.scheduledResume) {
                        statuses[site] = data[site].scheduledResume || 'none';
                    } else {
                        statuses[site] = data[site].paused ?
                            'disabled' : 'enabled';
                    }
                });
                return cb(null, statuses);
    ...
    deleteScheduledResumeService(details, cb) {
        ...
        return this._getZkStateNodeDetails(sites, (err, data) => {
            ...
            // if sites all do not have any scheduled resumes
            const noScheduledResumes = sites.every(site =>
                data[site].scheduledResume === null);
            if (noScheduledResumes) {
                this._logger.info('no scheduled resumes to cancel for ' +
                    `location(s): ${sites}`);
                const statuses = {};
                Object.keys(data).forEach(site => {
                    statuses[site] = null;
            ...
            const condition = { scheduledResume: false };
            return this._checkServiceStateStatus(sites, condition,
            (err, data) => {
                if (err) {
                    const failedSites = err.sites;
                    this._logger.error('failed to cancel scheduled resume for' +
                        ` location(s): ${failedSites}`, {
                            error: err,
                            method: 'BackbeatAPI.deleteScheduledResumeService',
                        });
                    return cb(err);
                }

                this._logger.info('deleted scheduled resume for location(s): ' +
                    `${sites}`);
                const statuses = {};
                Object.keys(data).forEach(site => {
                    statuses[site] = data[site].scheduledResume || 'none';
     ...
     * @return {undefined}
     */
    _getZkStateNodeDetails(sites, cb) {
        const stateBySite = {};
        async.each(sites, (site, next) => {
            const path =
                `${zookeeperReplicationNamespace}${zkCRRStatePath}/${site}`;
    ...
    getResumeCRRSchedule(details, cb) {
        ...
        this._getZkStateNodeDetails(sites, (err, data) => {
            if (err) {
                return cb(err);
            }
            const schedules = {};
