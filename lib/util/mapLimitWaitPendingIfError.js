...
async function mapLimitAsync(coll, limit, asyncIteratee) {
    const results = new Array(coll.length);
    let nextIdx = 0;
    let pendingError = null;

    async function worker() {
        while (nextIdx < coll.length && !pendingError) {
            const idx = nextIdx++;
            try {
                results[idx] = await asyncIteratee(coll[idx]);
            } catch (err) {
                if (!pendingError) {
                    pendingError = err;
                }
    ...
    return { err: pendingError, results };
}

module.exports = mapLimitWaitPendingIfError;
module.exports.mapLimitAsync = mapLimitAsync;
