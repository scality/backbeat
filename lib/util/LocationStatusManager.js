...
class LocationStatusManager {
    ...
    _handleScheduledResume(locations, cb) {
        async.eachLimit(locations, 10, (loc, next) => {
            const services = Object.keys(loc.value);
            services.forEach(svc => {
                if (loc.value[svc] && loc.value[svc].scheduledResume) {
                    this._scheduleResumeJob(loc._id, svc, new Date(loc.value[svc].scheduledResume));
                }
            });
            return next();
        }, err => {
    ...
    _deleteInvalidLocations(locations, cb) {
        const validLocationDetails = locations
        ...
        .then(() => cb(null, validLocationDetails))
        .catch(err => {
    ...
    _getMongoStateDetails(service, locations, cb) {
        this._locationStatusColl.find(
            { _id: { $in: locations } },
            { projection: { _id: 0, [`value.${service}`]: 1 } }
        )
        .toArray()
    ...
    _scheduleResumeJob(location, service, date) {
        function triggerResume() {
            const query = {
                ...
                [`value.${service}.scheduledResume`]: { $lte: date.toISOString() }
            };
            const update = {
                $set: {
                    [`value.${service}`]: {
    ...
    resumeService(service, locations, isScheduled, body, cb) {
        ...
        if (typeof isScheduled === 'boolean') {
            ...
            schedule = new Date();
            schedule.setMinutes(schedule.getMinutes() + hours);
        }
        ...
        return async.eachLimit(locations, 10, (location, next) => {
            ...
            if (schedule) {
                query.$or =  [
                    { [`value.${service}.scheduledResume`]: { $lte: schedule.toISOString() } },
                    { [`value.${service}.scheduledResume`]: { $eq: null } },
                ];
            }
            const update = {
                $set: {
