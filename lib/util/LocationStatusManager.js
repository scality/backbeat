...
class LocationStatusManager {
     ...
     * @param {string} location location name
     * @param {object} update conditional update query
     * @param {callback} cb callack
     * @returns {undefined}
     */
    _updateServiceStatusForLocation(location, update, cb) {
    ...
    _scheduleResumeJob(location, service, date) {
        function triggerResume() {
            ...
            this._updateServiceStatusForLocation(location, update, err => {
                if (err) {
    ...
    deleteScheduledResumeService(service, locations, cb) {
        return async.eachLimit(locations, 10, (location, next) => {
            ...
            this._deleteResumeJob(location, service);
            return this._updateServiceStatusForLocation(location, update, next);
        }, err => {
            if (err) {
    ...
    resumeService(service, locations, isScheduled, body, cb) {
        ...
        return async.eachLimit(locations, 10, (location, next) => {
            ...
            const update = {
                $set: {
                    [`value.${service}.paused`]: !schedule,
                    [`value.${service}.scheduledResume`]: schedule || null,
                }
            };
            return this._updateServiceStatusForLocation(location, update, err => {
                if (err) {
                    return next(err);
                }
                if (schedule) {
