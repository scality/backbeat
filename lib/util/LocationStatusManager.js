...
class LocationStatusManager {
     ...
     */
    _initCollection(cb) {
        return this._mongoClient.db().createCollection(locationStatusCollection)
            .then(() => {
                this._locationStatusColl = this._mongoClient.db().collection(locationStatusCollection);
                cb();
            })
            .catch(err => {
