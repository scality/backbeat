...
class LocationStatusManager {
    ...
    /**
     * Updates a location's pause/resume status in mongo
     * @param {string} location location name
     * @param {string} service service name
     * @param {object} conditions conditional update values
     * @param {object} newStatus new status values
     * @param {callback} cb callback
     * @returns {undefined}
     */
    _updateServiceStatusForLocation(location, service, conditions, newStatus, cb) {
        const query = { _id: location };
        Object.keys(conditions).forEach(key => {
            query[`value.${service}.${key}`] = conditions[key];
        });
        const update = {
            $set: {
    ...
    _scheduleResumeJob(location, service, date) {
        function triggerResume() {
            const conditions = { paused: true, scheduledResume: { $eq: date } };
            const newStatus = { paused: false, scheduledResume: null };
            this._updateServiceStatusForLocation(location, service, conditions, newStatus, err => {
                if (err) {
    ...
    deleteScheduledResumeService(service, locations, cb) {
        return async.eachLimit(locations, 10, (location, next) => {
            ...
            const conditions = { paused: true, scheduledResume: { $ne: null } };
            const newStatus = { paused: true, scheduledResume: null };
            this._deleteResumeJob(location, service);
            return this._updateServiceStatusForLocation(location, service, conditions, newStatus, next);
        }, err => {
            if (err) {
    ...
    resumeService(service, locations, isScheduled, body, cb) {
        ...
        return async.eachLimit(locations, 10, (location, next) => {
            ...
            const newStatus = schedule ?
                { paused: true, scheduledResume: schedule } :
                { paused: false, scheduledResume: null };
            return this._updateServiceStatusForLocation(location, service, conditions, newStatus, err => {
                if (err) {
                    return next(err);
                }
                if (schedule) {
