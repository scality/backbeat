...
class KafkaBacklogMetrics extends EventEmitter {
    ...
    isReady() {
        if (!(this._zookeeper &&
            ...
            this._log.error('KafkaBacklogMetrics is not ready, zookeeper not connected',
                { error: this._zookeeper.getState() });
            return false;
        }
        this._log.debug('KafkaBacklogMetrics is ready, zookeeper connected');
        return true;
    }
    ...
    _readOffset(topic, partition, offsetType, label, cb) {
        ...
        this._zookeeper.getData(zkPath, (err, offsetData, offsetStat) => {
            ...
            return cb(null, offset, lastModAgeMs);
        });
    }
                ...
                async.waterfall([
                    ...
                    (offset, next) => {
                        ...
                        consumerOffsets.forEach(consumerOffsetInfo => {
                            ...
                            if (consumerZkAgeMs !== undefined) {
                                info.consumerZkAgeMs = consumerZkAgeMs;
                            }
                            if (targetZkAgeMs !== undefined) {
                                info.targetZkAgeMs = targetZkAgeMs;
                            }
