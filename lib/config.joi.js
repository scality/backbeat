...
const joiSchema = joi.object({
    ...
    kafka: {
        ...
        backlogMetrics: {
            zkPath: joi.string().default('/backbeat/run/kafka-backlog-metrics')
                .meta({ env: 'KAFKA_BACKLOG_METRICS_ZKPATH' }),
            intervalS: joi.number().default(60)
                .meta({ env: 'KAFKA_BACKLOG_METRICS_INTERVALS' }),
        },
    ...
    s3: hostPortJoi.optional().keys({
        host: hostPortJoi.extract('host').meta({ env: 'CLOUDSERVER_HOST' }),
        port: hostPortJoi.extract('port').meta({ env: 'CLOUDSERVER_PORT' }),
    }),
    ...
    server: {
        healthChecks: joi.object({
            allowFrom: joi.array().items(joi.string()).default([])
                .meta({ env: 'HEALTHCHECKS_ALLOWFROM' }),
        }).required(),
