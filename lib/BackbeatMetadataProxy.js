...
class BackbeatMetadataProxy extends BackbeatTask {
    ...
    headObject(params, log, cb) {
        this.retry({
            actionDesc: 'head object from source',
            logFields: {
                bucket: params.bucket,
                objectKey: params.objectKey
            },
            actionFunc: done => this._headObjectOnce(params, log, done),
            shouldRetryFunc: err => err.retryable,
            log,
        }, cb);
    }

    _headObjectOnce(params, log, cb) {
        const { bucket, objectKey, locations } = params;
        log.debug('heading object', {
            where: 'source',
            bucket, objectKey, locations,
            method: 'BackbeatMetadataProxy._headObjectOnce',
        });
        ...
        req.send((err, data) => {
            if (err) {
                ...
                log.error('an error occurred during head object request', {
                    method: 'BackbeatMetadataProxy._headObjectOnce',
                    origin: 'source',
                    endpoint: this._s3Endpoint,
                    error: err,
