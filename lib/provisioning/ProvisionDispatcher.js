...
class ProvisionDispatcher {
    ...
    constructor(zkConfig) {
        console.log('creating provision dispatcher');
        console.log('here is the zkConfig passed', zkConfig);
        ...
        this._client.connect((err, res) => {
            console.log('trying to connect', err, res);
        });
    ...
    subscribe(cb) {
        ...
        this._waitConnected(() => {
            console.log('about to run async parallel stuff');
            async.parallel([
                done => this._registerLeader(done),
                done => this._registerOwner(done),
            ], err => {
                if (err) {
                    console.log('error during subscribe process');
                    return cb(err);
                }
                console.log('no error from waiting to connect');
    ...
    _redispatchProvisions() {
        ...
        return async.eachLimit(Object.keys(provisionsByOwner), 20,
            _provisionOwner);
    }
    ...
    _registerLeader(cb) {
        ...
        console.log('ZK PATH to register leaders', zkPath);
        this._client.create(
            zkPath, null,
            ...
            (err, _path) => {
                if (err) {
                    console.log('err while trying to register leader', err);
                    return cb(err);
                }
    ...
    _registerOwner(cb) {
        ...
        console.log('registering owner', zkPath);
        console.log(zookeeper.ACL.OPEN_ACL_UNSAFE);
        console.log(zookeeper.CreateMode.EPHEMERAL);
        this._client.create(
            zkPath, null, zookeeper.ACL.OPEN_ACL_UNSAFE,
            zookeeper.CreateMode.EPHEMERAL,
            err => {
                if (err) {
                    console.log('error trying to register owner', err);
                    return cb(err);
                }
                console.log('successfully register owner');
