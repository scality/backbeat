...
const constants = {
    ...
    dataStoreVersionId: 'test-object-dataStoreVersionId',
...
const mockSourceEntry = {
    ...
    getDataStoreVersionId: () => constants.dataStoreVersionId,
...
class S3Mock {
        ...
        if (req.method === 'GET') {
            if (req.url === '/zenko-source-bucket?location') {
                res.end('<?xml version="1.0" encoding="UTF-8"?>'
                        ...
                        + '</LocationConstraint>');
            } else if (req.url === `/zenko-source-bucket/${constants.bucket}/`
                       + `${constants.objectKey}?versionId=${constants.dataStoreVersionId}`) {
                if (this.testScenario === 'abortGet') {
                    log.info('aborting GET request');
                    res.socket.end();
                    res.socket.destroy();
    ...
    onRequestToTargetLocation(req, res) {
        if (req.method === 'GET') {
            ...
            req.on('end', () => {
                if (req.url === `/zenko-target-bucket/${constants.bucket}/`
                    + `${constants.objectKey}?uploads`) {
                    res.end('<?xml version="1.0" encoding="UTF-8"?>'
                ...
                } else if (req.url === `/zenko-target-bucket/${constants.bucket}/`
                          + `${constants.objectKey}?uploadId=test-upload-id`) {
                    res.setHeader('x-amz-version-id', 'target-version');
                    res.end('<?xml version="1.0" encoding="UTF-8"?>'
                            + '<CompleteMultipartUploadResult xmlns="http://s3.amazonaws.com/doc/2006-03-01/">'
                            + `<Location>http://127.0.0.4:7777/${constants.bucket}/`
                            + `${constants.objectKey}</Location>`
            ...
            req.on('end', () => {
                if (req.url === `/zenko-target-bucket/${constants.bucket}/`
                    ...
                    res.end();
                } else if (req.url.startsWith(`/zenko-target-bucket/${constants.bucket}/`
                                              + `${constants.objectKey}?partNumber=`)) {
                    res.setHeader('etag', 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa');
                    res.end();
                } else {
                    assert(false, `did not expect HTTP method ${req.method} on target location URL ${req.url}`);
