...
const { VaultServiceCredentials, newSTSClientFromVaultAuth } =
...
describe('newSTSClientFromVaultAuth', () => {
    it('should error if extension is invalid', () => {
        const extConfig = {
            auth: {
                transport: 'http',
                vault: { host: '127.0.0.1', port: 8600 },
            },
        };
        assert.throws(() => newSTSClientFromVaultAuth(extConfig));
    });

    it('should error if extension credentials are missing', () => {
        const extConfig = {
            auth: {
                transport: 'http',
                vault: {
                    host: '127.0.0.1', port: 8600,
                    serviceCredentials: { type: 'account', account: 'missing' },
                },
            },
        };
        assert.throws(() => newSTSClientFromVaultAuth(extConfig));
    ...
    it('should return client', () => {
        const extConfig = {
            auth: {
                ...
                vault: {
                    host: '127.0.0.1', port: 8600,
                    serviceCredentials: { type: 'account', account: 'lifecycle' },
                },
            },
        };
        const client = newSTSClientFromVaultAuth(extConfig);
        assert(client instanceof STS);
        assert.strictEqual(client.config.credentials.accessKeyId, 'lifecycleKey');
        assert.strictEqual(client.config.credentials.secretAccessKey, 'lifecycleSecret');
    });
});
