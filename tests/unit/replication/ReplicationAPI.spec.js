...
describe('ReplicationAPI', () => {
    ...
    describe('::sendDataMoverAction ', () => {
        ...
        it('stamps traceparent headers on the kafka entry when a span is active', done => {
            const transitionTime = new Date().toISOString();
            const action = ActionQueueEntry.create('copyLocation');
            action
                .setAttribute('target', {
            ...
            withActiveSpan(() => {
                ReplicationAPI.sendDataMoverAction(capturingProducer, action, fakeLogger, err => {
                    assert.ifError(err);
                    assert(Array.isArray(captured.headers));
