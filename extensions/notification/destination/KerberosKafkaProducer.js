...
const { gssapiSaslOption } = require('./saslGssapi');

const CONNECT_TIMEOUT_MS = 10000;
const AUTHENTICATION_TIMEOUT_MS = 10000;
const DEFAULT_DELIVERY_TIMEOUT_MS = 30000;

// kafkajs compresses in process and only bundles gzip; the other codecs need
// a companion package, so a config asking for one is refused up front rather
// than at the first send. Matched case insensitively, as librdkafka does.
...
/**
 * Producer to a kerberised external kafka destination, authenticating as the
 * destination's own principal.
 *
 * It exposes the part of the BackbeatProducer surface the notification
 * destinations and the delivery producer pool use: a 'ready' or 'error' event
 * after construction, send(entries, cb) called back on the delivery report,
 * and close(cb).
 *
 * The reason it exists alongside KafkaProducer is identity. node-rdkafka asks
 * GSSAPI for the process default credential, so a process can hold exactly one
 * kerberos identity and a second kerberised destination authenticates as the
 * first one's principal. This producer names the principal it wants, so one
 * delivery worker can serve destinations belonging to different principals.
 */
class KerberosKafkaProducer extends EventEmitter {
    /**
     * @constructor
    ...
    getConfigJoi() {
        return joi.object({
            ...
            topic: joi.string(),
            auth: authSchema,
            // accepted so that the pool can build either producer from the
            // same config; kafkajs has no poll loop to schedule
    ...
    getClientId() {
        return 'NotificationKerberosProducer';
    }
    ...
    setFromConfig(joiResult) {
        ...
        this._compression = COMPRESSION_TYPES[String(compressionType).toLowerCase()];
        if (this._compression === undefined) {
            throw new Error(`unsupported compressionType "${compressionType}" for a ` +
                'kerberos destination served by the pure JS producer, use none or gzip');
        }
        if (this._auth.type !== 'kerberos') {
            throw new Error('KerberosKafkaProducer needs an auth configuration of type kerberos');
        }
    }
     ...
     *
     * @return {Object|false} kafkajs ssl option
     */
    _sslOption() {
        ...
        const ssl = {};
        if (ca) {
            ssl.ca = [readOrThrow(ca, 'CA')];
        }
        if (client) {
            ssl.cert = readOrThrow(client, 'Client certificate');
        }
        if (key) {
            ssl.key = readOrThrow(key, 'Key');
            if (keyPassword) {
                ssl.passphrase = keyPassword;
            }
        }
        return ssl;
    }
    ...
    _connect() {
        ...
        try {
            ...
            client = new Kafka({
                ...
                authenticationTimeout: AUTHENTICATION_TIMEOUT_MS,
                logLevel: logLevel.WARN,
                logCreator: () => ({ namespace, label, log }) => {
                    this._log.warn('kafkajs', {
                        producer: this.getClientId(),
                        namespace,
                        level: label,
                        message: log.message,
                    });
        ...
        this._client = client;
        this._producer = client.producer({ allowAutoTopicCreation: false });
        this._producer.connect().then(() => {
            this._ready = true;
            this._log.info('connected to kerberised kafka destination', {
                method: 'KerberosKafkaProducer._connect',
                brokers: this._brokers,
                topic: this._topic,
                principal: this._auth.principal,
            });
    ...
    send(entries, deliveryReportsCb) {
        if (!this._topic) {
            process.nextTick(() => {
                this._log.error('no topic configured to send messages to', {
                    method: 'KerberosKafkaProducer.send',
                });
        ...
        }).then(reports => deliveryReportsCb(null, reports))
            .catch(err => {
        ...
        this._producer.disconnect()
            ...
            .catch(err => {
                this._log.error('error closing producer', {
                    method: 'KerberosKafkaProducer.close',
                    error: err.message,
                });
                cb(err);
            });
...
module.exports = KerberosKafkaProducer;
