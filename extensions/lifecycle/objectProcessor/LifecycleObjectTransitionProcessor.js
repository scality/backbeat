...
class LifecycleObjectTransitionProcessor extends LifecycleObjectProcessor {
    ...
    getConsumerParams() {
        ...
        this._lcConfig.coldStorageTopics.forEach(topic => {
            ...
            const coldLocation =
                topic.slice(this._lcConfig.coldStorageStatusTopicPrefix.length);

            assert(locations[coldLocation],
                `${coldLocation}: unknown location`);

            assert(locations[coldLocation].isCold,
                `${coldLocation} is not a valid cold storage location`);
