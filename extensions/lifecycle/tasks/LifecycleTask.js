...
class LifecycleTask extends BackbeatTask {
    ...
    _getRules(bucketData, bucketLCRules, object, log, done) {
        if (this._isDeleteMarker(object)) {
            ...
            const filteredRules = filterRules(bucketLCRules, object, []);
            return done(null, getApplicableRules(filteredRules, object, this._lifecycleDateTime));
        }
        ...
        return req.send((err, tags) => {
            ...
            const filteredRules = filterRules(bucketLCRules, object, tags);
    ...
    _applyTransitionRule(params, log) {
        async.waterfall([
            ...
            (entry, next) => ReplicationAPI.sendDataMoverAction(
    ...
    _compareMPUUploads(bucketData, bucketLCRules, uploads, log) {
        uploads.forEach(upload => {
            ...
            const noTags = { TagSet: [] };
            const filteredRules = filterRules(bucketLCRules, upload, noTags);
            let aRules = getApplicableRules(filteredRules, {}, this._lifecycleDateTime);
