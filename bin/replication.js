...
program
    ...
    .option('--source-profile <name>',
            '[required] source aws/credentials profile')
    .option('--target-bucket <name>', '[optional] target bucket name ' +
            '(necessary if adding a Zenko site as a replication endpoint)')
    ...
    .action(options => {
        ...
        const hasValidOptionals = (targetBucket || targetProfile) ?
            ...
            multiSites.split(',').includes(destinationEndpoint.site))) : true;
